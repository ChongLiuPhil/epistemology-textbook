...
def main() -> int:
    ...
    for path in FILES:
        ...
        for env in ENVIRONMENTS:
            ...
            if begins != ends:
                errors.append(
                    f"{rel}: unbalanced math environment {env}: {begins} begin / {ends} end"
                )
        ...
        for number, match in enumerate(DISPLAY.finditer(body), start=1):
            ...
            multiline = "\\\\" in block or any(
                f"\\begin{{{env}}}" in block for env in ENVIRONMENTS
            )
            if hint > 96 and not multiline:
                warnings.append(
                    f"{rel}: display equation #{number} may be too wide (hint {hint})"
                )
    ...
    print(
        f"Math layout source check: {len(FILES)} QMD files, "
